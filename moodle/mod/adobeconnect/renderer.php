...
//            $html .= html_writer::tag('label', $meetingdetail->url, $param);
		...
		if(has_capability('mod/adobeconnect:addinstance', $context)){
        ...
        foreach ($recordings as $key => $recordinggrp) {
            if (!empty($recordinggrp)) {
                ...
                foreach ($recordinggrp as $recording_scoid => $recording) {
                    ...
                    if(!startsWith($recording->name, '/') || $recording->duration ){
                        ...
                        if($recording->startdate == 0 || $recording->startdate == null ) {
                            $table->data[] = array($c,
                                $link,
                                userdate(strtotime($recording->createdate)),
                                '',
                                ''
                            );
                        } else {
